...
func detectMCPs(claudeDir string, transcriptPath string) []string {
	...
	if cwd != "" {
		local := readSettings(filepath.Join(cwd, ".claude", "settings.local.json"))
		for name := range local.MCPServers {
			allNames = append(allNames, name)
		}
	}
...
func collectDisabledMCPs(proj claudeProjectConfig) map[string]bool {
	all := make([]string, 0, len(proj.DisabledMCPServers)+len(proj.DisabledMcpjsonServers))
	all = append(all, proj.DisabledMCPServers...)
	all = append(all, proj.DisabledMcpjsonServers...)
	if len(all) == 0 {
		return nil
	}
